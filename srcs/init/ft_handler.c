...
static inline void		ft_init_handler(struct termios *cpy_term, t_env **env)
{
	struct winsize	size;
...
static inline void		ft_quit(struct termios cpy_term, t_env **env, int mes)
{
	ft_reset_copy(*env, 0);
	if (mes == 1)
...
static inline void		ft_else(struct termios cpy_term, t_env **env,
